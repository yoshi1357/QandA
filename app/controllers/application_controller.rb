class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters
    devise_parameter_sanitizer.permit(:sign_up, keys: [:name]) # 新規登録時(sign_up時)にnameというキーのパラメーターを追加で許可する
 	  ...
 	  devise_parameter_sanitizer.permit(:sign_up, keys: [:image_cache]) # 新規登録時(sign_up時)にimage_cacheというキーのパラメーターを追加で許可する
    ...
    devise_parameter_sanitizer.permit(:account_update, keys: [:name]) # 編集時(edit時)にnameというキーのパラメーターを追加で許可する
 	  ...
 	  devise_parameter_sanitizer.permit(:account_update, keys: [:image_cache]) # 編集時(edit時)にimage_cacheというキーのパラメーターを追加で許可する
  end
